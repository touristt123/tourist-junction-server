...
async function handleFinalizePackageBookings(req, res) {
    try {
        ...
        const primaryDriverResponse = await sendSms(updatedPackageBooking.primaryDriver?.mobileNumber, `You have been selected for a trip with ${updatedPackageBooking.agencyName}. Departure Date: ${formatDate(updatedPackageBooking.departureDate)} Vehicle Number:${updatedPackageBooking.vehicle.number} Customer Name:${updatedPackageBooking.customerName} Customer Contact:${updatedPackageBooking.mobileNumber} Pick-Up Point:${updatedPackageBooking.pickupPoint} Route: ${updatedPackageBooking.departurePlace} to ${updatedPackageBooking.destinationPlace} Pick-Up Time:${formatTime(updatedPackageBooking.departureTime)} Please reach the bus parking area 2 hours before departure and check the vehicle. Kindly start the trip from your app before departure. All driver and cleaner details for this trip: Name:${updatedPackageBooking.primaryDriver?.name}, Mobile:${updatedPackageBooking.primaryDriver?.mobileNumber} Name:${updatedPackageBooking.secondaryDriver?.name}, Mobile:${updatedPackageBooking.secondaryDriver?.mobileNumber} Name:${updatedPackageBooking.cleaner?.name} Mobile:${updatedPackageBooking.cleaner?.mobileNumber} Best regards, TOURIST JUNCTION PRIVATE LIMITED`, process.env.DLT_PACKAGE_BOOKING_FINALIZATION_DRIVER_TEMPLATE_ID)

        const secondaryDriverResponse = await sendSms(updatedPackageBooking.secondaryDriver?.mobileNumber, `You have been selected for a trip with ${updatedPackageBooking.agencyName}. Departure Date: ${formatDate(updatedPackageBooking.departureDate)} Vehicle Number:${updatedPackageBooking.vehicle.number} Customer Name:${updatedPackageBooking.customerName} Customer Contact:${updatedPackageBooking.mobileNumber} Pick-Up Point:${updatedPackageBooking.pickupPoint} Route: ${updatedPackageBooking.departurePlace} to ${updatedPackageBooking.destinationPlace} Pick-Up Time:${formatTime(updatedPackageBooking.departureTime)} Please reach the bus parking area 2 hours before departure and check the vehicle. Kindly start the trip from your app before departure. All driver and cleaner details for this trip: Name:${updatedPackageBooking.primaryDriver?.name}, Mobile:${updatedPackageBooking.primaryDriver?.mobileNumber} Name:${updatedPackageBooking.secondaryDriver?.name}, Mobile:${updatedPackageBooking.secondaryDriver?.mobileNumber} Name:${updatedPackageBooking.cleaner?.name} Mobile:${updatedPackageBooking.cleaner?.mobileNumber} Best regards, TOURIST JUNCTION PRIVATE LIMITED`, process.env.DLT_PACKAGE_BOOKING_FINALIZATION_DRIVER_TEMPLATE_ID)
        
        const cleanerResponse = await sendSms(updatedPackageBooking.cleaner?.mobileNumber, `You have been selected for a trip with ${updatedPackageBooking.agencyName}. Departure Date: ${formatDate(updatedPackageBooking.departureDate)} Vehicle Number:${updatedPackageBooking.vehicle.number} Customer Name:${updatedPackageBooking.customerName} Customer Contact:${updatedPackageBooking.mobileNumber} Pick-Up Point:${updatedPackageBooking.pickupPoint} Route: ${updatedPackageBooking.departurePlace} to ${updatedPackageBooking.destinationPlace} Pick-Up Time:${formatTime(updatedPackageBooking.departureTime)} Please reach the bus parking area 2 hours before departure and check the vehicle. Kindly start the trip from your app before departure. All driver and cleaner details for this trip: Name:${updatedPackageBooking.primaryDriver?.name}, Mobile:${updatedPackageBooking.primaryDriver?.mobileNumber} Name:${updatedPackageBooking.secondaryDriver?.name}, Mobile:${updatedPackageBooking.secondaryDriver?.mobileNumber} Name:${updatedPackageBooking.cleaner?.name} Mobile:${updatedPackageBooking.cleaner?.mobileNumber} Best regards, TOURIST JUNCTION PRIVATE LIMITED`, process.env.DLT_PACKAGE_BOOKING_FINALIZATION_DRIVER_TEMPLATE_ID)
...
async function handleUpdatePackageBooking(req, res) {
    try {
        ...
        const { vehicleId, otherVehicleId, customerName, customerNo, mobileNumber, officeAddress, kmStarting, perKmRateInINR, advanceAmountInINR, remainingAmountInINR, advancePlace, departurePlace, destinationPlace, departureTime, departureDate, returnTime, returnDate, tollInINR, otherStateTaxInINR, note } = req.body
        // console.log({ vehicleId, otherVehicleId, customerName, mobileNumber, alternateNumber, kmStarting, perKmRateInINR, advanceAmountInINR, remainingAmountInINR, advancePlace, departurePlace, destinationPlace, departureTime, departureDate, returnTime, returnDate, tollInINR, otherStateTaxInINR, note });
        if (!vehicleId || !otherVehicleId || !customerName || !officeAddress || !mobileNumber || !kmStarting || !perKmRateInINR || !advanceAmountInINR || !remainingAmountInINR || !advancePlace || !departurePlace || !destinationPlace || !departureTime || !returnTime || !tollInINR || !otherStateTaxInINR || !note) {
            return res.status(400).json({
                success: false,
                message: "Provide all the fields"
            })
